...
var app = {
		...
		if (!user.isWanderer) {
			if (actualResult == 'win') {
  			user.win(user.info.id, group, user.source).then((response) => {
	        	  ...
	        	  user.sendEmail(user.info.id, 'DentSu Coupon Link', emailContent);
						}
	 ...
	 /* var enableAgreeCheckbox = false;
      ...
      var email = document.getElementById('emailInput').value;
        ...
        if (response.data.status == true) {
        	...
        	var emailContent = '<head><meta charset="utf-8"></head>ご登録ありがとうございました。下記にあるリンクをクリックしてください。その後キャンペーンへの参加をお願いします<br><br><a href="https://rmarepo.richmediaads.com/o2o/dentsu/demo/index.html?userId=' + email + '" target="_blank">https://rmarepo.richmediaads.com/o2o/dentsu/demo/index.html?userId=' + email + '</a>';
        	user.sendEmail(email, 'DentSu Survey Link', emailContent);
        	// user.trackRegister();
        }
	...
	setQuestions() {
	  ...
	  this.q[4] = new singleAnswerQuestion({
	  	...
	  	question: '<span class="red">QUESTION 4</span><br>普段どんなシャンプーを利用していますか？',
	  	answers: [{
	    	value: 'ボタニカルシャンプー',
	    	text: 'ボタニカルシャンプー',
	    }, {
	  ...
	  this.q[5] = new singleAnswerQuestion({
	  	...
	  	question: '<span class="red">QUESTION 5</span><br>普段シャンプーはどこで買いますか？。',
	  	answers: [{
	    	value: 'コンビニエンスストア',
	    	text: 'コンビニエンスストア',
	    }, {
